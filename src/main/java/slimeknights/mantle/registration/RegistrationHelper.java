...
public class RegistrationHelper {
  ...
  // TODO PORT (later stage): NeoForge 1.21 removed MissingMappingsEvent / registry remapping entirely
  // (no MissingMappingsEvent.java in net.neoforged.neoforge.registries on the 1.21.x branch). There is
  // currently no direct replacement for remapping legacy registry IDs, so the handleMissingMappings(...)
  // helper that previously lived here has been removed. Any caller that relied on it must be updated
  // (or the remapping feature dropped) once a NeoForge replacement is chosen.
