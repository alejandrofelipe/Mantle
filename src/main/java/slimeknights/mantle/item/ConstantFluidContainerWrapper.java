...
/**
 * Represents a fluid handler for a container item with a single constant fluid that becomes empty once drained.
 * <p>
 * In NeoForge this is no longer an {@code ICapabilityProvider}; register it centrally via
 * {@code event.registerItem(Capabilities.FluidHandler.ITEM, (stack, ctx) -> new ConstantFluidContainerWrapper(fluid, stack), THE_ITEM)}
 * on {@code RegisterCapabilitiesEvent} (see {@link ContainerFoodItem.FluidContainerFoodItem}).
 */
